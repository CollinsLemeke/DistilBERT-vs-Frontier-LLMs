...
# decide later which one to feed DistilBERT.
...
# Before decide the drop strategy, one last check: rows where review exists
...
# this thesis is about reducing carbon cost, we should pick the SMALLEST max_length that
...
# measure the real subword-token counts.
...
# we don't have to re-read 80 MB from Drive.
...
# Visualise the LLM preview as a chart
...
# Pairwise agreement between the LLMs themselves
...
# get to the baseline comparison section, we don't have to re-derive it.
...
# Verify token length distribution AFTER tokenization
...
# Launch training with CodeCarbon tracking -
...
# -Test set evaluation with inference emissions tracking -
...
# ---Save best model + tokenizer + metrics summary to Drive ----
import json
...
# - Instantiate the Task B model + Trainer --
...
# ---Launch Task B training with CodeCarbon ---
from codecarbon import EmissionsTracker
...
# Task B test evaluation with CodeCarbon tracking
...
# Confusion matrix + store predictions
print("Generating predictions for test set...")
...
# LLM head-to-head comparison (Task A + Task B)
...
# Save all comparison artefacts to Drive
...
# Final check + download-ready confirmation
print("=" * 70)
...
path = "/content/drive/MyDrive/Colab Notebooks/DistilBERT/Carbon Aware Sentiment Analysis at Scale.ipynb"
...
# These are REAL user reviews from MHARD with their REAL user-given star ratings.
...
RANDOM_SEED_FOR_DISPLAY = 42
NUM_SAMPLES = 50

# Sample random indices from the test set
rng = np.random.RandomState(RANDOM_SEED_FOR_DISPLAY)
sample_indices = rng.choice(len(test_df), NUM_SAMPLES, replace=False)
sample_indices = sorted(sample_indices)  # nicer ordering in display

# Helper: map a star rating to its sentiment class (matches MHARD collapse rule)
...
with torch.no_grad():
    for idx in sample_indices:
        ...
        true_star = int(row["rating"])  # original 1..5 user rating
        ...
        top_prob  = float(probs[pred_star - 1])
        diff = abs(pred_star - true_star)
        match = "Correct" if diff == 0 else f"Off by {diff}"

        # Truncate text for table readability
        text_short = text if len(text) <= 70 else text[:67] + "..."

        rows.append({
            "Review (cleaned)": text_short,
            "Ground truth": f"{true_star} stars",
            "Predicted": f"{pred_star} stars",
            "Confidence": f"{top_prob:.3f}",
            "Result": match,
            "Sentiment": star_to_sentiment(pred_star),
        })

df_testset = pd.DataFrame(rows)

# Summary
correct_count = sum(1 for r in rows if r["Result"] == "Correct")
off_by_one    = sum(1 for r in rows if r["Result"] == "Off by 1")
total         = len(rows)

print("=" * 70)
print("QUALITATIVE INFERENCE ON RANDOM TEST-SET REVIEWS")
print("=" * 70)
print(f"Source         : MHARD held-out test set (20,082 reviews)")
print(f"Random seed    : {RANDOM_SEED_FOR_DISPLAY} (reproducible)")
...
print(f"Within ±1      : {correct_count + off_by_one} / {total} "
      f"({100*(correct_count+off_by_one)/total:.1f}%)")
print()

# Same styled display
def colour_result(val):
    if val == "Correct":
        return "background-color: #D8F3DC; color: #1B4332; font-weight: bold"
    elif val.startswith("Off by 1"):
